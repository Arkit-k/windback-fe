...
Most API endpoints accept either a JWT token or an API secret key.
...
4. **Use strong passwords** for your account.

See [Data Security](/docs/data-security) for details on how customer data is protected.
...
API requests are rate limited per IP and per API key to prevent abuse. If you hit rate limits, back off and retry after a short delay.
